$(function(){
	...
	$('#searchDo').tap(function(){
		...
		if(location.href.indexOf('/list/') < 0){	// 判断当前是否在list目录
			location.href = './list/search.php?keyword=' + encodeURIComponent($('#search').val());
		}else{
			if(location.href.indexOf('search') < 0){	// 判断当前是否未搜索页
				location.href = './search.php?keyword=' + encodeURIComponent($('#search').val());
			}else{
				location.replace('./search.php?keyword=' + encodeURIComponent($('#search').val()));
			}
		}
	})
	// 回车搜索
...
function parseTemplate(template,data){
	...
	if(data.id){
		if(location.href.indexOf('/list/') < 0){	// 判断当前是否在list目录
			data.href = './play.php?vid=' + encodeURIComponent(data.id);
		}else{
			data.href = '../play.php?vid=' + encodeURIComponent(data.id);
		}
	}
